...
var Hook = function () {
  ...
  _createClass(Hook, [{
    ...
    value: function process(thing) {
      ...
      return Promise.resolve(this.preProcess(thing)).then(function (preProcessResult) {
        if (preProcessResult) {
          if (_this.flags.execute) {
            return Promise.resolve(_this.execute(thing));
          }
        }
        ...
        if (_this.flags.hook && _this.isHook(_this.hook)) {
          return Promise.resolve(_this.hook.process(thing));
        }
      }).then(function () {
        // fire the post process as appropriate
        if (_this.flags.postProcess) {
          return Promise.resolve(_this.postProcess(thing));
        }
      });
    }
  }, {
      ...
      return Promise.resolve(this.preProcess(thing)).then(function (preProcessResult) {
        if (preProcessResult) {
          if (_this3.flags.execute) {
            return Promise.resolve(_this3.execute(thing));
          }
        }
        return Promise.resolve(true);
      }).then(function () {
        ...
        if (_this3.flags.hook && _this3.isHook(_this3.hook)) {
          return Promise.resolve(_this3.hook.process(thing));
        }
      }).then(function () {
