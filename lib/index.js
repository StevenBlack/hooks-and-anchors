...
var Hook = function () {
  ...
  _createClass(Hook, [{
    ...
    value: function process(thing) {
      ...
      var implementation = function implementation(thing2) {
        var postProc = [],
            proc = [];
        _this.loadP(proc, postProc);

        var prom = proc.shift()(thing2);
        proc.forEach(function (func) {
          prom = prom.then(function () {
            return func(thing2);
          });
        });
        postProc.forEach(function (func) {
          prom = prom.then(function () {
            return func(thing2);
          });
        });
      ...
      return implementation(thing).catch(function (err) {
        console.log(err);
      });
    }
  }, {
    ...
    value: function setHook(hook) {
      ...
      if (this.isHook(this.hook)) {
        ...
        (_hook = this.hook).setHook.apply(_hook, [hook].concat(otherArgs));
      } else {
        // hook could be a package name
        if (typeof hook === 'string') {
          return this.setHook(this.classInstanceFromString.apply(this, [hook].concat(otherArgs)));
        }
    ...
    value: function classInstanceFromString(packageLocation) {
      ...
      return new (Function.prototype.bind.apply(Temp, [null].concat(otherArgs)))();
    }
  }, {
    key: 'loadP',
    value: function loadP(proc, postProc) {
